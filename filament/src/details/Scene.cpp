...
void FScene::prepare(utils::JobSystem& js,
    ...
    // TODO: use linear allocator for the temporary vectors below

    using RenderableInstanceContainer = FixedCapacityVector<
            std::pair<RenderableManager::Instance, TransformManager::Instance>,
            std::allocator< std::pair<RenderableManager::Instance, TransformManager::Instance> >,
            false>;

    using LightInstanceContainer = FixedCapacityVector<
            std::pair<LightManager::Instance, TransformManager::Instance>,
            std::allocator< std::pair<LightManager::Instance, TransformManager::Instance> >,
            false>;

    RenderableInstanceContainer renderableInstances{
            RenderableInstanceContainer::with_capacity(entities.size()) };

    LightInstanceContainer lightInstances{
            LightInstanceContainer::with_capacity(entities.size()) };
    ...
    auto* lightJob = jobs::parallel_for(js, rootJob,
            ...
            std::cref(lightWork), jobs::CountSplitter<64, 4>());
    ...
    if (auto [li, ti] = directionalLightInstances ; li) {
        ...
        lightData.elementAt<FScene::POSITION_RADIUS>(0) = float4{ 0, 0, 0, inf };
